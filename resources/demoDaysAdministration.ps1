...
## start SSH on VMHosts
Get-VMHost $arrVMHosts | Get-VMHostService | ?{$_.Key -eq "TSM-SSH"} | ?{$_.Running -eq $false} | Start-VMHostService -Confirm:$false
## stop SSH on VMHosts
Get-VMHost $arrVMHosts | Get-VMHostService | ?{$_.Key -eq "TSM-SSH"} | ?{$_.Running -eq $true} | Stop-VMHostService -Confirm:$false


## get creds for ESXi hosts
$credESXiRoot = Get-Credential root

## cmd to add the given line to the config file
#$strCmdToRun = "echo 'vhv.allow = ```"TRUE```"' >> /etc/vmware/config"
## cmd to echo out the config file
$strCmdToRun = "cat /etc/vmware/config"
$arrVMHosts | %{plink.exe -l root -pw $credESXiRoot.GetNetworkCredential().Password $_ $strCmdToRun}
...
## added a couple of add'l portgroups
